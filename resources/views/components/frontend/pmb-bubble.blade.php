...
<a
    href="{{ $url }}"
    class="group fixed bottom-5 right-5 z-40 flex max-w-[calc(100vw-2.5rem)] items-center gap-3 rounded-full border border-white/70 bg-white/95 p-2 pr-4 text-slate-950 shadow-[0_18px_50px_rgba(15,23,42,.18)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-emerald-100 hover:shadow-[0_24px_60px_rgba(4,120,87,.22)] focus:outline-none focus:ring-4 focus:ring-emerald-200 sm:bottom-7 sm:right-7 sm:pr-5"
    aria-label="Daftar PMB Universitas Aisyiyah Yogyakarta"
>
    <span class="relative grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-full bg-white p-1.5 text-white shadow-lg shadow-emerald-900/20 ring-1 ring-emerald-100 transition duration-300 group-hover:scale-105 sm:h-14 sm:w-14">
        @if($logoUrl)
    ...
    <span class="min-w-0">
        <span class="block text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700">PMB Unisa</span>
        <span class="mt-0.5 flex items-center gap-2 text-sm font-bold leading-tight text-slate-950 sm:text-[15px]">
            Mau daftar? Klik di sini
            <svg class="h-4 w-4 shrink-0 -rotate-45 text-emerald-700 transition duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
